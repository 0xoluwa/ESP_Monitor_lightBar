...
void fsm_tick(void){
    /* Phase 1: walk and update counters under the lock */
    ...
    while (curr != NULL) {
        if (--curr->down_counter == 0U) {
            if (curr->interval != 0U) {
                curr->down_counter = curr->interval;
            }
            if (n_expired < MAX_FSM_TIMERS) {
    ...
    for (int i = 0; i < n_expired; i++) {
        fsm_post((fsm *)expired[i]->state_machine, (fsm_event *)expired[i]);
    }
}
